...
class AppTest(unittest.TestCase):
    ...
    def test_print_hello_success(self):
        ...
        self.assertEqual(404, response.status_code, "Erro no test_http_code!")

    def test_print_hello_error (self):
        response = self.app.get('/name')
        self.assertNotEqual("Nome não informado", response.get_data(as_text=True)
                          , "Erro no test_print_hello!")